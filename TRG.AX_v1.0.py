...
while (len(df_date) < 30):
    if (td.weekday() in [5, 6]):
        ...
        td = td + datetime.timedelta(days=1)
        df_date = df_date.append(pd.DataFrame([td]))

df_date.columns = ["Date"]
df_output = df_date.assign(Prediction = df_output)
date = datetime.now().strftime("%d%m")
df_output.to_csv(f'output/TGR.AX_Prd_{date}.csv', index = False, header=True)
